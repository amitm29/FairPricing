import { getGdpMultiplier, GDP_DATA, GDP_SOURCE } from '@/lib/conversion-indexes/gdp';
  ...
  if (pppResult.status === 'rejected') {
    ...
    for (const [regionCode, entry] of Object.entries(PRICING_INDEX)) {
      staticData[regionCode] = {
        ...
        gdpMultiplier: getGdpMultiplier(regionCode).multiplier,
        gdpFallback: getGdpMultiplier(regionCode).fallback,
        gdpYear: GDP_DATA[regionCode]?.year,
