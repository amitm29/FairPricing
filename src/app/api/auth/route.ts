...
export async function POST(request: NextRequest) {
  if (!isEncryptionAvailable()) {
    return NextResponse.json(
      { error: 'Store connections are unavailable because server session encryption is not configured. The server administrator must set ENCRYPTION_KEY. Your credentials have not been checked.' },
      { status: 503 }
    );
  }
  try {
