    ...
    (async () => {
      try {
        ...
        const tasks = entries.map(
          ([territoryCode, { targetPrice, currency, maxPrice }]) =>
            async () => {
              ...
              return { territoryCode, targetPrice, currency, pricePoints: pricePoints.filter(p=>Number(p.customerPrice) <= (maxPrice ?? Infinity)) };
            }
        );
