...
type ProductWithApple = { _appleProduct?: { id?: string; baseTerritory?: string } };
...
export function BulkPricingModal({
  ...
  const draftKey = pricingDraftKey(platform, platform === 'apple' ? (bundleId || (product as ProductWithApple)._appleProduct?.id || product.sku) : product.packageName, 'product', product.sku);
  ...
  const [rounding, setRounding] = useSavedPricingSetting<RoundingMode>(draftKey, 'rounding',
  ...
  const { prices: previewPrices, error: previewError } = useMemo(() => {
    ...
    const calculatedPrices = calculateConnectedPrices(
      overrides,
      basePriceNum,
      targetRegions,
      strategy,
      ...
      { ...pricingOptions, snapToTiers: platform === 'apple' }
    );
  ...
  const executeApply = async () => {
    ...
    previewPrices.forEach((calculated) => {
      if (selectedRegions.has(calculated.regionCode) || calculated.regionCode === appleBaseRegion) {
        prices[calculated.regionCode] = platform === 'apple' && pricingOptions.capAtBase ? {...calculated.price, maxPrice: calculated.rawPrice} : calculated.price;
      }
    });
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="space-y-6 py-4">
          ...
          <AdvancedPricingControls strategy={strategy} rounding={rounding} setRounding={setRounding} options={pricingOptions} setOptions={setPricingOptions} apple={platform === 'apple'} /><PriceMapPreview rows={previewPrices.map(r=>({code:r.regionCode,currency:r.currencyCode,price:r.rawPrice,ratio:r.rawPrice/(r.adjustedUsdPrice/r.multiplier*r.exchangeRate)}))}/>
                        ...
                        >
                          ...
                          <TableCell className="text-sm">
                            <div className="flex flex-col">
                              ...
                              {isRequired && (
                                <span className="rounded-full bg-primary/10 px-1.5 py-0.5 text-[10px] font-medium tracking-wide text-primary uppercase">Base region</span>
                              )}
                            </div>
                          </TableCell>
            ...
            <DialogHeader className="flex-shrink-0 text-left">
              ...
              <DialogDescription asChild>
                <div className="text-sm text-muted-foreground">
                  ...
                  {platform === 'apple' && (
                    <div className="mt-2 font-medium text-warning">
