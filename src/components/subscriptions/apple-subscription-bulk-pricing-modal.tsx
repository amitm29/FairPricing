...
export function AppleSubscriptionBulkPricingModal({
  ...
  const draftKey = pricingDraftKey('apple', bundleId || subscription.id, 'subscription', subscription.id);
  ...
  const [rounding, setRounding] = useSavedPricingSetting<RoundingMode>(draftKey, 'rounding', 'nearest-tier');
  ...
  const { prices: previewPrices, error: previewError } = useMemo(() => {
    try {
      const calculatePreview = (): PreviewPrice[] => {
    ...
    const calculatedPrices = calculateConnectedPrices(
      ...
      { ...pricingOptions, snapToTiers: true }
    );
    ...
    return calculatedPrices.map((calculated) => {
      ...
      return {
        ...
        multiplier: tierPrice / (calculated.adjustedUsdPrice / calculated.multiplier * calculated.exchangeRate),
                            ...
                            <TableRow key={preview.territoryCode} className={rowClassName}>
                              ...
                              <TableCell className="text-right text-sm">
                                <Tooltip>
                                  ...
                                  {preview.multiplierSource === 'gdp' && 'GDP per capita (PPP estimate when unavailable)'}
                                  {preview.multiplierSource === 'blend' && 'Weighted index blend'}
