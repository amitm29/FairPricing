...
import { CURRENCY_PRICE_TIERS, findClosestTierForCurrency, hasTierData } from './price-tier-data';
...
export async function resolvePPPPricesToPricePoints(
  credentials: AppleConnectCredentials,
  inAppPurchaseId: string,
  prices: Record<string, { currencyCode: string; units: string; nanos?: number; maxPrice?: number }>
...
async function resolvePPPPricesWithCache(
  ...
  for (const [territoryCode, price] of territories) {
    ...
    const allowedTiers = CURRENCY_PRICE_TIERS[currency]?.filter(t=>t.price <= (price.maxPrice ?? Infinity));
    const closestTier = price.maxPrice === undefined ? findClosestTierForCurrency(localAmount, currency) : allowedTiers?.reduce<(typeof allowedTiers)[number] | undefined>((best,t)=>!best || Math.abs(t.price-localAmount)<Math.abs(best.price-localAmount)?t:best,undefined);
...
async function resolvePPPPricesFromAPI(
  ...
  for (const [territoryCode, price] of territories) {
    ...
    const closest = findClosestPricePoint(localAmount, localPricePoints, price.maxPrice);
    if (!closest) {
...
export function findClosestPricePoint(
  ...
  pricePoints = pricePoints.filter(p=>Number.isFinite(Number(p.customerPrice)) && Number(p.customerPrice)>0 && Number(p.customerPrice)<=maxPrice);
